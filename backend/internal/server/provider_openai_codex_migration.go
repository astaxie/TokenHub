...
func init() {
	registerProviderAdapterStartupMigration(normalizeOpenAIToCodexProviderAdapters)
}

func normalizeOpenAIToCodexProviderAdapters(ctx context.Context, tx *gorm.DB) error {
	var providers []Provider
	if err := tx.WithContext(ctx).Where("type = ?", ProviderOpenAI).Find(&providers).Error; err != nil {
		return err
	}
	for _, provider := range providers {
		var resources []ProviderResource
		if err := tx.WithContext(ctx).Where("provider_id = ?", provider.ID).Find(&resources).Error; err != nil {
			return err
		}
		...
		if !hasDirectResource && strings.TrimSpace(provider.APIKey) == "" {
			...
			if err := tx.WithContext(ctx).Model(&Provider{}).Where("id = ?", provider.ID).Updates(updates).Error; err != nil {
				return err
			}
			continue
		}
		if err := splitMixedOpenAIProvider(tx.WithContext(ctx), provider, subscriptions); err != nil {
			return err
		}
	}
	return nil
