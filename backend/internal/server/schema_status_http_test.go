...
func TestAdminSchemaStatusEndpoint(t *testing.T) {
	...
	if !payload.Ready || payload.SchemaVersion != 4 {
		t.Fatalf("expected ready expanded status, got %+v", payload)
	}
	if payload.Compatibility.TargetVersion != 4 || payload.Compatibility.MinCompatible != 1 || payload.Compatibility.MaxCompatible != 5 {
