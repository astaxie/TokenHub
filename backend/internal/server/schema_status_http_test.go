...
func TestAdminSchemaStatusEndpoint(t *testing.T) {
	...
	if !payload.Ready || payload.SchemaVersion != 7 {
		t.Fatalf("expected ready expanded status, got %+v", payload)
	}
	if payload.Compatibility.TargetVersion != 8 || payload.Compatibility.MinCompatible != 7 || payload.Compatibility.MaxCompatible != 8 {
