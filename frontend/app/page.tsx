...
const translations: Record<Exclude<AppLanguage, "zh-CN">, Record<string, string>> = {
  en: {
    ...
    "添加成员": "Add Member",
    "暂无项目成员": "No project members yet",
    "项目角色": "Project Role",
    "允许发 Key": "Allow Key Issuance",
    "分配项目成员": "Assign Project Member",
    "owner/maintainer 可管理项目 Key；developer 可发放自己的 Key；viewer 只用于可见和统计。": "owner/maintainer can manage project Keys; developer can issue their own Keys; viewer is for visibility and stats only.",
    "需要单独给 viewer 或特殊成员开放发 Key 时启用。": "Enable when you need to grant Key issuance to a viewer or a specific member individually.",
    "把用户分配到一个或多个项目空间，并控制项目内 Key 发放权限。": "Assign users to one or more project spaces and control Key issuance permissions within the project.",
    "发 Key": "Issue Key",
    "维护者": "Maintainer",
    "开发者": "Developer",
    "只读": "Read-only",
  ...
  ja: {
    ...
    "项目成员": "プロジェクトメンバー",
    "添加成员": "メンバーを追加",
    "暂无项目成员": "プロジェクトメンバーはまだいません",
    "项目角色": "プロジェクトロール",
    "允许发 Key": "Key 発行を許可",
    "分配项目成员": "プロジェクトメンバーを割り当て",
    "owner/maintainer 可管理项目 Key；developer 可发放自己的 Key；viewer 只用于可见和统计。": "owner/maintainer はプロジェクト Key を管理でき、developer は自分の Key を発行でき、viewer は表示と統計のみに使用します。",
    "需要单独给 viewer 或特殊成员开放发 Key 时启用。": "viewer や特定のメンバーに個別に Key 発行を許可する場合に有効にします。",
    "把用户分配到一个或多个项目空间，并控制项目内 Key 发放权限。": "ユーザーを 1 つ以上のプロジェクトスペースに割り当て、プロジェクト内の Key 発行権限を制御します。",
    "发 Key": "Key 発行",
    "维护者": "メンテナー",
    "开发者": "開発者",
    "只读": "閲覧のみ",
