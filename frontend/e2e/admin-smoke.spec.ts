...
test.describe("Billing workflow", () => {
  ...
  test("admin can confirm model prices and export statements without connectors", async ({ page }) => {
    await login(page);
    ...
    await page.getByRole("tab", { name: "模型调价", exact: true }).click();
    await page.getByRole("combobox", { name: "计价对象", exact: true }).selectOption({ index: 1 });
    const selected = await page.getByRole("combobox", { name: "计价对象", exact: true }).inputValue();
    await page.getByLabel("普通输入", { exact: true }).fill("2");
    await page.getByLabel("输出", { exact: true }).fill("6");
    expect(await page.getByLabel("普通输入", { exact: true }).evaluate((el) => el.getBoundingClientRect().height)).toBeGreaterThanOrEqual(40);
    ...
    await expect(page.getByRole("button", { name: "应用到模型价格" })).not.toBeVisible();
    ...
    await page.getByRole("button", { name: "应用到模型价格", exact: true }).click();
    ...
    await page.getByRole("button", { name: "应用到模型价格", exact: true }).click();
    await dialog.getByRole("button", { name: "确认应用价格", exact: true }).click();
    await expect(page.getByRole("status").filter({ hasText: "模型价格已更新" })).toBeVisible();
    await expect(dialog).not.toBeVisible();
    await page.getByRole("tab", { name: "调价记录", exact: true }).click();
    await expect(page.locator(".billing-version").first()).toContainText(selected);
    await page.getByRole("tab", { name: "模型调价", exact: true }).click();
    await page.getByRole("combobox", { name: "计价对象", exact: true }).selectOption("");
    await page.getByRole("combobox", { name: "计价对象", exact: true }).selectOption(selected);
    await expect(page.getByLabel("普通输入", { exact: true }).first()).toHaveValue("2");
    await page.getByRole("tab", { name: "平台账单", exact: true }).click();
    ...
    expect(download.suggestedFilename()).toBe("tokenhub-provider-statement.csv");
    ...
    const bottom = await last.boundingBox(); const top = await next.boundingBox();
