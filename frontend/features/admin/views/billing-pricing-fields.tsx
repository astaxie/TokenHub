...
export function RateFields({ rates, update, cache = false }: { rates: Rates; update: (key: keyof Rates, value: string) => void; cache?: boolean }) {
  return <div className="billing-form-grid">{rateLabels.filter(([key]) => cache ? key.startsWith("cache_") : !key.startsWith("cache_")).map(([key, label]) => <Field key={key} label={label}><input inputMode="decimal" placeholder={cache ? tx("留空按基础价格计算") : "0.00"} value={rates[key]} onChange={(event) => update(key, event.target.value)} /></Field>)}</div>;
}
export function completedRates(rates: Rates): Rates {
