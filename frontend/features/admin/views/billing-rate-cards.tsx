        ...
        <form className="billing-editor" onSubmit={(event) => { event.preventDefault(); void act("preview"); }} onChange={invalidate}>
          <fieldset disabled={busy || loadingModel} className="billing-form-content">
            ...
            <section className="billing-step">
              ...
              <details className="billing-disclosure"><summary>{tx("单独设置缓存价格")}<small>{tx("可选")}</small></summary><p className="billing-hint">{tx(card.kind === "tenant" ? "缓存价留空使用模型默认规则；填写 0 表示明确免费。" : "上游缓存价格未知时请留空；已知时填写单价，0 表示明确免费。")}</p><RateFields cache inherit={card.kind === "tenant"} rates={card.rates} update={(key, value) => updateCard({ ...card, rates: { ...card.rates, [key]: value } })} /></details>
              {card.kind === "tenant" ? <p className="billing-hint">{tx("已带入模型当前价格。空白缓存项保持默认规则，试算与实际收费使用同一计价逻辑。")}</p> : null}
