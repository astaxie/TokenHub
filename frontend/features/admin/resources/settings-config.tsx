...
export function systemSettingConfig(): ResourceConfig<AdminResource> {
  const base = genericResourceConfig("settings", "系统设置", "网关地址、审计保留、企业集成和默认策略", [
    ...
    { key: "max_json_request_bytes", label: "JSON 请求体上限", type: "number", placeholder: "8388608", help: "常规 JSON 请求体最大字节数；留空用默认 8 MiB，上限 512 MiB。" },
    { key: "max_multimodal_request_bytes", label: "多模态请求体上限", type: "number", placeholder: "33554432", help: "含图像等多模态内容的请求体最大字节数；留空用默认 32 MiB，上限 512 MiB。" },
  ]);
  return {
    ...base,
    eyebrow: "基础设置",
