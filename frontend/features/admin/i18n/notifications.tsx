export const notificationTranslations: Record<"en" | "ja" | "ru", Record<string, string>> = {
  en: {
  ...
  ru: {
    "SMTP 加密": "SMTP-шифрование",
    "auto：按服务器能力机会式升级 STARTTLS（legacy）；starttls：强制 STARTTLS，服务器不支持时拒绝发送（端口 587）；ssl：从第一个字节即隐式 TLS（端口 465）。": "auto: оппортунистическое обновление до STARTTLS при поддержке сервером (legacy); starttls: обязательный STARTTLS, отклонение при отсутствии поддержки (порт 587); ssl: неявный TLS с первого байта (порт 465).",
  },
};
