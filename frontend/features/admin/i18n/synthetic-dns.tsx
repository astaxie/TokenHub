const syntheticDNSTranslations = {
  ...
  ru: {
    "允许 Provider 使用 Synthetic DNS / Fake-IP": "Разрешить провайдерам использовать Synthetic DNS / Fake-IP",
    "仅当 TokenHub 所在主机启用了 Fake-IP 代理，且 Provider 域名因此解析到合成地址时开启。该例外只用于域名解析结果，不允许把字面量 IP 直接配置为 Provider Base URL。": "Включайте только когда хост TokenHub использует Fake-IP прокси и домены провайдеров разрешаются в синтетические адреса. Исключение применяется только к результатам DNS; литеральный IP по-прежнему нельзя использовать как Base URL провайдера.",
    "允许信任私网 / ULA Synthetic DNS 网段（高风险）": "Доверять диапазонам приватных / ULA Synthetic DNS (высокий риск)",
    "默认仍禁止 RFC1918 私网和 IPv6 ULA。仅当代理确实把这些范围用作 Fake-IP 池（例如部分 Xray IPv6 配置）时开启；开启后，恶意或被劫持的 Provider 域名可能访问该范围内的真实内网服务。": "Приватные сети RFC1918 и IPv6 ULA по умолчанию остаются заблокированными. Включайте только если прокси действительно использует эти диапазоны как Fake-IP пул. После включения вредоносный провайдер может получить доступ к реальным внутренним сервисам.",
    "Synthetic DNS / Fake-IP 网段": "Диапазоны Synthetic DNS / Fake-IP",
    "安全提示：每行一个 CIDR，必须与本机代理的实际 Fake-IP 池一致。198.18.0.0/15 是基准测试保留网段，只是常被代理软件用作 Fake-IP，并非 Fake-IP 专属；不同软件或配置可使用其他网段。配置过宽或填入真实内网会削弱 SSRF 防护。loopback、link-local、metadata、multicast、NAT64 等敏感范围始终禁止。": "Предупреждение: вводите по одному CIDR на строку, соответствующему фактическому Fake-IP пулу прокси. Слишком широкие диапазоны или реальные внутренние сети ослабляют защиту от SSRF. Диапазоны loopback, link-local, metadata, multicast и NAT64 всегда заблокированы.",
    "开启时至少填写一个 Synthetic DNS CIDR。": "При включении укажите хотя бы один Synthetic DNS CIDR.",
    "Synthetic DNS CIDR 格式无效，请每行填写一个 CIDR。": "Недопустимый формат Synthetic DNS CIDR. Вводите по одному CIDR на строку.",
    "Synthetic DNS CIDR 范围过宽，请缩小后重试。": "Диапазон Synthetic DNS CIDR слишком широк. Сузьте его и повторите.",
    "该 Synthetic DNS CIDR 与受保护地址范围重叠，无法保存。": "Этот Synthetic DNS CIDR пересекается с защищённым диапазоном адресов и не может быть сохранён.",
    "该 Synthetic DNS CIDR 属于私网或 ULA；如确认代理使用此网段，请先开启高风险私网信任。": "Этот Synthetic DNS CIDR относится к приватной сети или ULA. Если прокси действительно использует его, сначала включите доверие к приватным диапазонам (высокий риск).",
  },
};
