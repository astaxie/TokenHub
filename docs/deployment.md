...
| `TOKENHUB_PROVIDER_UPSTREAM_ACCESS_MODE` | `auto` | `auto` permits administrator-configured local and private upstreams, including private DNS names. `strict` retains legacy literal-only exceptions. Unknown values use strict behavior |
...
| `TOKENHUB_PROVIDER_UPSTREAM_ALLOWED_CIDRS` | empty | Optional restrictive private CIDR list. In auto mode an empty list allows RFC1918/ULA; a nonempty list restricts both private literals and private DNS results. In strict mode the list permits private literals only. Invalid nonempty entries do not enable unrestricted access. Special-use addresses cannot be allowlisted |
...
| `TOKENHUB_PROVIDER_UPSTREAM_ALLOW_LOOPBACK` | `false` (legacy) | Effective in strict mode or with a nonempty private allowlist. There, `true` permits localhost/127.0.0.1/::1. Auto mode without a private allowlist allows loopback regardless of the old template value; use strict mode to retain a deliberate loopback denial |
...
In the default auto mode, administrators can enter HTTP endpoints such as `http://127.0.0.1:8000/v1`, a private IPv4/IPv6 address, `host.docker.internal`, a Docker service name, or an enterprise DNS name without configuring a CIDR allowlist. The address must be reachable from the backend. A container's loopback address refers to that container, not its host. TokenHub does not create DNS entries, Docker network attachments, or host aliases.

HTTP hostnames must resolve entirely to permitted local addresses before saving and before sending a request. Public or mixed public/private HTTP results are rejected before any credentials or body are sent. Direct connections use the validated addresses without a second lookup; proxy requests keep the original Host and TLS server name. Metadata, link-local, multicast and other protected special-use targets remain denied. Redirects may keep the same scheme and authority, including a local same-origin redirect; cross-origin redirects remain denied.

Nonempty existing private allowlists remain restrictive. Old templates included `TOKENHUB_PROVIDER_UPSTREAM_ALLOW_LOOPBACK=false` even for ordinary deployments; that value alone no longer opts out of auto mode. Operators who intentionally require the previous boundary can set `TOKENHUB_PROVIDER_UPSTREAM_ACCESS_MODE=strict`, retain their literal CIDRs and loopback setting, and separately set `TOKENHUB_PROVIDER_UPSTREAM_PROXY_LOCAL=true` when local traffic must follow the selected proxy. These deployment settings require a backend restart or container recreation.
