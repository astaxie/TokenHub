...
| `TOKENHUB_PROVIDER_UPSTREAM_ACCESS_MODE` | `strict` | `strict` permits only explicit literal-address exceptions. Set `auto` to allow administrator-configured local and private upstreams, including private DNS names. Unknown values use strict behavior |
...
Strict mode is the default. Operators must set `TOKENHUB_PROVIDER_UPSTREAM_ACCESS_MODE=auto` to opt into automatic local and private access. Nonempty private allowlists remain restrictive in either mode, and strict mode continues to honor explicit literal CIDRs and `TOKENHUB_PROVIDER_UPSTREAM_ALLOW_LOOPBACK`. Set `TOKENHUB_PROVIDER_UPSTREAM_PROXY_LOCAL=true` separately when local traffic must follow the selected proxy. These deployment settings require a backend restart or container recreation.
