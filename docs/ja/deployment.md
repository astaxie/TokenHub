...
以下のコマンドは Docker Compose を使用します。どちらのバックエンドも Docker なしで同様にサポートされます。[ベアメタルデプロイ](#ベアメタルデプロイdocker-なし)を参照してください。
...
`deploy/bare-metal/` は、Docker を一切使わずに同じ 2 つのサービスを実行します。Docker が利用できない、または許可されていない環境で使用してください。アプリケーション自体は変更していません。バックエンドは単一の Go バイナリ、コンソールは Next.js standalone サーバーです。

入口は 2 つあります:

| | 用途 | root 権限 | 再起動後も残る |
| --- | --- | --- | --- |
| `run-local.sh` | 自分のマシンで本番ビルドを実行する | 不要 | いいえ |
| `build.sh` + `install.sh` | サーバーに systemd でインストールする | 必要 | はい |

どちらも **SQLite 専用**です。バックエンド自体は PostgreSQL にも対応していますが、これらのスクリプトは管理しません。後述の[データベース](#データベース)を参照してください。
...
必要に応じて両コンポーネントをビルドし、ループバック上で実行します。バイナリ、コンソールバンドル、データベース、ログ、pid ファイルはすべてリポジトリ内の `.tokenhub/`（gitignore 済み）に置かれ、そのディレクトリを削除すればすべての痕跡が消えます。システム全体へのインストールもサービスアカウントの作成も行いません。

`-d` を付けるとサービスは起動元のシェルから切り離され、シェルの終了後もターミナルを閉じた後も動き続けます。ただし再起動後は自動で復帰しません。それが必要な場合は systemd でのインストールを使用してください。どちらのモードでも pid ファイルを記録するため、`status` と `stop` はフォアグラウンドのインスタンスにも有効です。`stop` はシグナルを送る前に記録された pid が本当にこのインスタンスのものかを検証するので、再利用された pid を誤って kill することはありません。また起動前に両方のポートを確保するため、既に他のサービスが応答しているポートを「起動成功」と誤認することもありません。

以上は Linux で検証済みです。macOS には `setsid` がないため、停止時はプロセスツリーをたどる方式にフォールバックします。この経路は実装済みですが macOS 上では未検証です。

実行されるのは dev サーバーではなく**本番ビルド**（デプロイ時と同じ standalone バンドル）なので、本番ビルドでのみ現れる問題を検出できます。開発用の資格情報（`admin` / `admin123456`）を使用し、ループバックのみにバインドするため、ローカル利用専用でありデプロイ手段ではありません。
...
| ホスト | 要件 |
| --- | --- |
| ビルドホスト | Go（バージョンは `backend/go.mod` を参照）、Node 22 以上、npm、C コンパイラ |
| ターゲットホスト | systemd と GNU coreutils/findutils を備えた Linux（主要ディストリビューションはすべて該当）、システム全体にインストールされた Node 22 以上 |

バックエンドは cgo 経由で SQLite をリンクするため C コンパイラが必要で、生成されるバイナリはターゲットのアーキテクチャと libc に依存します。ターゲットと一致するホストでビルドしてください。

Node はシステム全体にインストールする必要があります。サービスユニットは `ProtectHome=true` を設定しているため、ユーザーのホームディレクトリ配下のインタプリタ（nvm、fnm、asdf）は実行時に参照できません。インストーラーは後で失敗するサービスを作る代わりに、その場で拒否します。
...
`dist/tokenhub-<version>-<os>-<arch>/` と対応する `.tar.gz` を生成します。このディレクトリは自己完結型で、バックエンドバイナリ、コンソールバンドル、両方のカタログ、ユニットファイル、設定例、インストーラー、`SHA256SUMS` を含みます。いずれかの形式をターゲットホストにコピーしてください。ターゲット側にリポジトリのチェックアウトや Go ツールチェーンは不要です。
...
`--generate-secrets` は、新規作成された設定の `TOKENHUB_SECRET_KEY`、`TOKENHUB_ADMIN_TOKEN`、`TOKENHUB_BOOTSTRAP_ADMIN_PASSWORD` を生成値で埋めます。指定しない場合、`change-me-*` のプレースホルダーが残っている設定では、両ユニットはインストールされますが **disabled のまま起動されません**。設定が無効なユニットを有効化すると、次回起動時にクラッシュループになるためです。

起動後、インストーラーは `/readyz` とコンソールのポートをポーリングし、いずれかが失敗した場合は `systemctl status` と直近のジャーナルを表示します。

その他のオプション: `--no-start`、`--install-timers`（設定中のデータベース向けバックアップタイマー）、`--skip-backup`（アップグレード前スナップショットの省略、非推奨）。

### インストール後の構成

| パス | 所有者 | 内容 |
| --- | --- | --- |
| `/opt/tokenhub/releases/<version>/` | `root:root` | 変更しないリリースペイロード |
| `/opt/tokenhub/current` | シンボリックリンク | 有効なリリース。アトミックに切り替え |
| `/etc/tokenhub/backend.env` | `root:tokenhub` 0640 | バックエンド設定とシークレット |
| `/etc/tokenhub/frontend.env` | `root:tokenhub-web` 0640 | コンソール設定。シークレットは含まない |
| `/var/lib/tokenhub/` | `tokenhub:tokenhub` 0750 | SQLite データベース、バックアップ、アップグレード前スナップショット |

サービスアカウントは 2 つです。バックエンドが `tokenhub`、コンソールが `tokenhub-web`。設定ファイルを共有しないため、コンソールが侵害されてもシークレットキー、管理トークン、データベース資格情報は読み取れません。バックエンドユニットは `UMask=0077` で動作し、SQLite データベースとそのバックアップを非公開に保ちます。

### データベース

データベースは `/var/lib/tokenhub/tokenhub.db`、バックアップは `/var/lib/tokenhub/backups` に置かれます。`backend.env` では意図的に絶対パスを使用しています。組み込みのデフォルト値は作業ディレクトリからの相対パスであり、ここでの作業ディレクトリは読み取り専用のリリースディレクトリだからです。

バックエンドは PostgreSQL 上でも動作しますが、インストーラーはその構成を中途半端にサポートするのではなく**拒否**します。PostgreSQL を適切に管理するには、事前チェック、アップグレード前の `pg_dump`、バックアップの保持ポリシー、そしてサーバーのバージョンに追随する必要があるクライアントツールチェーンまで面倒を見る必要があり、いずれもこのインストーラーの責務ではありません。TokenHub を PostgreSQL で運用する場合は、バックエンドを自分で設定・管理してください。[PostgreSQL セットアップガイド](../postgresql-setup.md)を参照してください。

### バックアップ

`--install-timers` は日次タイマーをインストールします。バックエンド自身のオンラインバックアップ API を呼び出し、その後、期限切れのバックアップを削除します。アプリケーションは有効期限を記録するだけで自動削除は行わないため、保持ポリシーはタイマーが担います。稼働中の SQLite ファイルを直接コピーしてはいけません。壊れたコピーになります。バックアップ中はバックエンドの唯一の SQLite 接続を占有するため既定では負荷の低い時間帯に実行されます。また静的な管理トークンは最初に作成された管理者ユーザーとして認証されるため、そのユーザーを降格するとタイマーが動作しなくなります。

タイマーは失敗時に非ゼロで終了しジャーナルに記録します。`systemctl --failed` を監視するか、`OnFailure=` ユニットを追加してください。

リストアは、保存済みのプロバイダー資格情報を復号する元の `TOKENHUB_SECRET_KEY` がなければ意味を持ちません。このキーは別途バックアップしてください。データベースと同じディスク上のバックアップはローカル復旧用のコピーであり、災害対策にはなりません。別ホストへコピーしてください。

### アップグレードとロールバック

新しいリリースから `install.sh` を実行します。ペイロードのステージング、チェックサム検証、両サービスの停止、**検証済みのアップグレード前データベーススナップショット**の取得、`current` のアトミックな切り替え、再起動を行います。直前のリリースは保持されます。

このスナップショットは任意の保険ではありません。起動時に `AutoMigrate` が無条件で実行されるため、`current` を古いリリースに戻してもスキーマは**戻りません**。ロールバックにはスナップショットのリストアが必要です。サービス停止後に `-wal`、`-shm`、`-journal` の付随ファイルが残っている場合、データベースがクリーンに閉じられていないためスナップショットは中止されます。

### アンインストール

```bash
sudo /opt/tokenhub/current/uninstall.sh            # /etc/tokenhub と /var/lib/tokenhub を保持
sudo /opt/tokenhub/current/uninstall.sh --purge    # データベースとローカルバックアップも削除
```

### サービスの公開

Compose デプロイと同様に、両サービスは既定ですべてのインターフェースにバインドします。単一マシンでの試用を超える用途では、TLS を備えたリバースプロキシの背後に配置し、ファイアウォールでポートを制限し、`TOKENHUB_TRUSTED_PROXY_CIDRS` にプロキシのアドレスを設定してください。

`frontend.env` の `TOKENHUB_API_BASE_URL` はページに埋め込まれ、Node プロセスではなく**ブラウザ**から呼び出されます:

- `http://127.0.0.1:8080` はサーバー上でコンソールを開く場合にのみ機能します。
- HTTPS で配信されるコンソールから HTTP の API は呼び出せません。ブラウザが混在コンテンツをブロックします。
- `TOKENHUB_CORS_ALLOWED_ORIGINS` にはコンソールの正確なオリジン（スキーム、ホスト、ポート。パスは含めない）を指定する必要があります。
- 管理セッションはこの URL をブラウザのローカルストレージにキャッシュするため、値を変更しても既存セッションはログアウトまたはサイトデータの消去まで影響を受けません。
