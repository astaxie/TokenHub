...
バックグラウンドジョブ契約は、クォータ更新、アカウント同期、クリーンアップ、レポート、ヘルスチェックなど、モデルリクエスト経路に置くべきでない処理を対象とします。登録済みのプロセス内ジョブは宣言したスケジュールで実行するか、プラグイン管理画面から管理者が手動で実行できますが、外部コマンドジョブは現在登録できません。

> **ランタイムでの利用可否:** このページは外部バックグラウンドジョブ契約を定義します。パッケージと schedule 宣言は検証できますが、ホストレベルの隔離をまだ強制できないため、現在の TokenHub ランタイムは外部バックグラウンドコマンドを起動前に拒否します。プロセス内の組み込みジョブには影響しません。
...
フィクスチャを別のプラグインワークスペースへコピーし、Manifest、ハンドラー検証、Schema、テストを一緒に更新します。Devkit の出力で宣言したジョブ契約を検証してください。現在の TokenHub リリースへインストールした後も、プラグインの**詳細**ページにある折りたたみ済みの**開発者情報**で宣言を確認できますが、パッケージは**起動失敗**となり、スケジューラーはジョブを登録または実行しません。宣言したスケジュールと管理者実行コントロールは、将来の外部ランタイム契約だけを表します。
