...
コンソールには配信成功または機密情報を除去した失敗理由を表示します。HTTP 200 でも、DingTalk、Feishu、WeCom のアプリケーションエラーは失敗として扱います。HTTP 応答が不完全または読み取り不能の場合もテストは失敗します。業務結果の検証が必要なボットの応答本文が 4096 バイトを超える場合は失敗とし、汎用 Webhook の応答本文にはこの検証上限を適用しません。成功は SMTP サーバーまたは送信先 API がリクエストを受け付けたことを意味します。対象の受信トレイ／チャンネルで受信を確認し、メールの場合は迷惑メールも確認してください。各配信試行は**アラート配信**と監査ログに記録され、運用アラートは作成されません。テストを実行できるのはプラットフォーム管理者とセキュリティ管理者のみです。
