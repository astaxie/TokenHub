...
Key 単位の **Usage** ページでは、保存済み Key ID を利用量推移、モデル別・エラー別内訳、リクエスト明細の厳密な境界として使用します。ローテーション関係は案内のみで、前後の Key の利用量は合算しません。現在の日次・月次 Key クォータカードは設定済みのクォータタイムゾーンを使い、Gateway の Key 単位の admission チェックと同じグローバル、Project、Team、Key の上限を解決します。ユーザー集約クォータは別途適用され、この Key 単位の画面には含まれません。プラットフォーム管理者には Provider と Resource の集約パフォーマンスも表示されます。他のロールには既存のリクエスト詳細権限がそのまま適用され、Provider の実コストはプラットフォーム管理者だけが確認できます。
...
**System Settings > Gateway Base Settings > Quota Timezone**（`cfg_gateway` の `quota_timezone`）に `Asia/Shanghai` などの IANA タイムゾーンを設定します。既定は `UTC` で、既存環境でフィールドが未設定または空の場合も UTC を使用します。`TZ` と `dashboard_timezone` はこの設定を上書きしません。日次リクエスト数、Token、コストのクォータはローカル午前0時に、月次クォータは月初のローカル午前0時にリセットされ、夏時間も考慮されます。API Key の上限、継承した上限、ユーザー集約クォータ、および現在の使用量カードに適用されます。分単位のレート制限と請求・予算期間の UTC 境界は変わりません。

変更は新しい admission と現在の使用量表示に直ちに適用されます。実行中のリクエストとバックグラウンドジョブの精算、返金、復旧では admission 時に保存したバケットを使用し、スナップショットがない旧リクエストは UTC を使用します。既存カウンターは再配分されないため、切り替え時の期間が短くなったり長くなったり、既存の日付・月バケットを再利用したりする場合があります。低トラフィック時に切り替え、上限と現在の消費量を確認してください。UTC 以外を選ぶ前に全バックエンドレプリカを更新してください。未対応バージョンへ戻す際は、先に UTC に戻して未完了の処理を完了させてください。
...
**ヘルスとアラート → 通知チャンネル**でチャンネルを保存した後、行の**テスト通知を送信**をクリックします。TokenHub は保存済みの送信先と認証情報を使い、SMTP の受信者または設定された Webhook／ボットへテストと明記したメッセージを送信します。フォームの未保存の変更は使用しません。無効なチャンネルも、有効化せずにテストできます。

コンソールには配信成功または機密情報を除去した失敗理由を表示します。HTTP 200 でも、DingTalk、Feishu、WeCom は明示的な成功コード 0 を返す必要があります。アプリケーションエラー、結果コードの欠落や不正な値、不完全または読み取り不能な HTTP 応答は失敗として扱います。業務結果の検証が必要なボットの応答本文が 4096 バイトを超える場合は失敗とし、汎用 Webhook の応答本文にはこの検証上限を適用しません。成功は SMTP サーバーまたは送信先 API がリクエストを受け付けたことを意味します。対象の受信トレイ／チャンネルで受信を確認し、メールの場合は迷惑メールも確認してください。各配信試行は**アラート配信**と監査ログに記録され、運用アラートは作成されません。テストを実行できるのはプラットフォーム管理者とセキュリティ管理者のみです。

通知配信では HTTP リダイレクトを追跡しません。最終的な送信先 URL を直接設定してください。SMTP 配信は DATA の終了後にサーバーがメールを受け付けた時点で成功となり、その後の QUIT の失敗やタイムアウトでは結果を変更しません。これらの受け付け判定ルールは、テストと通常の通知の両方に適用されます。

API：`POST /api/admin/resources/notification-channels/{channel_id}/test`。管理者認証が必要で、リクエスト本文は不要です。`AlertDelivery` を返すため、`status`（`success` または `failed`）と `error` を確認してください。送信先が拒否しても、完了した試行は HTTP 200 を返します。SMTP と HTTP 配信にはネットワークタイムアウトを設定しています。応答、記録、監査では、保存された認証情報と認証情報を含む URL のパス／クエリを引き続きマスクします。
