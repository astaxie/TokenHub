...
Set **System Settings > Gateway Base Settings > Quota Timezone** (`quota_timezone` on `cfg_gateway`) to an IANA timezone such as `Asia/Shanghai`. The default is `UTC`, including installations where this field is missing or empty. `TZ` and `dashboard_timezone` do not override it. Daily request, token and cost quotas reset at local midnight; monthly quotas reset at local midnight on the first day of the month, including daylight-saving transitions. This applies to API Key limits, inherited limits and aggregate user quotas, and their current-usage cards. Minute rate limits and billing/budget periods retain their existing UTC boundaries.

Changes apply to new admissions and current-usage views immediately. In-flight requests and background job settlement, refunds and recovery retain the buckets captured at admission; older admissions without a snapshot retain UTC. Existing counters are not redistributed, so a timezone change can shorten or extend the transition period or reuse an existing date/month bucket. Plan the switch during a quiet period and review limits and current consumption. Upgrade every backend replica before selecting a non-UTC timezone; restore UTC and drain pending work before rolling back to a version without this setting.
