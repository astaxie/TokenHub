...
Administrators can connect a Super Grok / Grok CLI account as an `xai_grok` Provider. Authorization uses xAI device code rather than the Codex localhost callback. TokenHub sends chat traffic to `https://cli-chat-proxy.grok.com/v1` with Grok CLI identity headers. Official xAI API keys remain on `openai_compatible`. See [Use Super Grok Subscription from TokenHub](super-grok-subscription.md) for the operator flow, supported endpoints, limits, and how callers use an isolated Grok CLI home.
