...
`gpt-image-2` 通常是独立的 OpenAI API 模型，必须配置 `openai` 类型 Provider、API Key 和模型路由。它调用 Provider 的标准 `/v1/images/generations` 与 `/v1/images/edits`，不会选择 Codex 订阅账号或消耗 Codex 额度。唯一例外是带 Codex `originator` 或 `x-codex-image-turn-id` 请求头的生图和 JSON 图片编辑请求：TokenHub 会将其映射为 `codex-gpt-image-2` 并返回 `b64_json`，API Key 必须允许 `codex-gpt-image-2`。
...
| 图片编辑输入限制 | multipart 和原生 Codex JSON 都最多 16 张输入图；解码后单图最多 50 MB；整个请求最多 128 MB；`mask` 不计入 16 张输入图 |
...
- WebP
- 单张不超过 50 MB
- multipart 和原生 Codex JSON 都最多 16 张输入图，`mask` 不计入
- 整个请求不超过 128 MB

### 7.1 原生 Codex JSON 参考图

原生 Codex 请求使用 `application/json`，而不是 multipart：
...
请求必须包含非空 `x-codex-image-turn-id`，或将 `originator` 设置为以 `codex` 开头的值。`response_format` 会被忽略，返回中固定使用 `data[].b64_json`。

### 7.2 单张 multipart 参考图
...
| 400 | `too_many_images` | 编辑请求超过 16 张输入图 | 减少输入图数量；`mask` 不计入 |
...
| 413 | `image_edit_request_too_large` | 图片编辑请求超过 128 MB | 缩小请求体或减少输入图 |
