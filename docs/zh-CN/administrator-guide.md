...
控制台显示投递成功或脱敏后的失败原因。即使 HTTP 状态为 200，钉钉、飞书和企业微信返回的业务错误也会判定失败；HTTP 响应不完整或读取失败时，测试同样失败。需要校验业务结果的机器人响应正文超过 4096 字节时会判定失败，普通 Webhook 响应正文不受此校验大小限制。成功仅表示 SMTP 服务器或目标 API 已接受请求；请到目标收件箱／渠道确认接收，邮件还应检查垃圾邮件过滤。每次投递尝试都会写入**告警投递**及审计日志，不会创建正式告警。仅平台管理员和安全管理员可以测试。
