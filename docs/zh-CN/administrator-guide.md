...
在管理控制台的邮件渠道表单中，**SMTP 加密** 下拉框提供 `auto`（机会式 STARTTLS，legacy 默认）、`starttls`（强制 STARTTLS，服务器不支持时拒绝发送，端口 587）与 `ssl`（从第一个字节即隐式 TLS，端口 465）三个选项；新渠道默认 `starttls`，创建和更新时会将该值写入 `smtp_encryption` 字段。
