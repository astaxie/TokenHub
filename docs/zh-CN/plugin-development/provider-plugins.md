...
TokenHub 会把配置的 `provider-catalog.json` 中全部 159 个条目都表示为内置 Provider 插件包。每个包都有可检查的 Manifest、README、许可证和目录元数据。目录插件负责厂商身份、新增页元数据、详情页和生命周期；Host Adapter 负责 `OpenAI-Compatible` 等可执行协议。因此，多个目录插件可以共享一个适配器而无需复制运行代码。禁用目录插件会从新增 Provider 页面移除对应厂商，重新启用后立即恢复。
