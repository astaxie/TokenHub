...
Authentication, model allowlists, quotas, scoped routing, provider resource capacity, text preflight policies, response hooks, and usage attribution apply. Request hooks see JSON/text fields; multipart file bytes are opaque. Direct media audit records contain model, content type, and byte count, not uploaded/generated media. Client cookies and authorization are not forwarded; configured provider credentials and guarded outbound transport are used. An ambiguous direct-media submission failure does not trigger an automatic second generation on another route.
