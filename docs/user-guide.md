...
All four endpoints require a TokenHub project API key. An administrator must explicitly enable `codex_voice` on both the project and the key, and configure an active `codex-voice` route to an OpenAI Codex Provider backed by Codex Subscription account resources. The capability defaults to disabled; a missing project or key grant returns `model_not_allowed` without contacting an upstream.

Call creation uses the normal TokenHub admission, project-scoped routing, account capacity, failover, request logging, metrics, and tracing lifecycle. It consumes request, RPM, and concurrency quotas, but does not create token or cost usage records because the upstream call-create response has no authoritative token usage. Sideband attachment rechecks the same key, capability, and persisted call binding without creating a second billable request. TokenHub persistently binds the returned call ID to the selected subscription resource for one hour. The WebRTC media path is negotiated by the client and OpenAI; TokenHub proxies call bootstrap and sideband, not RTP media. Both call-create paths map to the ChatGPT Codex product backend; the overlapping `/v1/realtime/calls` name does not expose the complete public Realtime API.
...
A successful proxy cannot grant Voice entitlement. Upstream authorization and experimental session parameters are validated independently, so `403 Voice session access denied` can indicate account or workspace access restrictions, or an unsupported model and voice combination; diagnose it against the exact request and current Codex contract.
